...
$( document ).ready(function() {
    ...
    $(".header-" + pathname).addClass("active");
...
function showModal(modalID, datatable = null){
    ...
    $( ".container" ).append(addOverlay(overlaycount, window.getComputedStyle(modal).zIndex));
    overlaycount++;
...
function addOverlay(id, zIndex){
    // console.log("adding overlay")
    var overlayIndex = zIndex - 1;
    return `<div class="page-overlay" id="overlay`+id+`" style="z-index: `+overlayIndex+`"></div>`;
...
function closeModal(modalID, formReset = null, fieldReset = null){
    var modal = document.getElementById(modalID);
    modal.style.display = "none";
    overlaycount--;
    $( "#overlay"  + overlaycount).remove();
...
function submitForm(formUpdate){
    ...
    else if (formUpdate == "#addPatientForm"){
        var formData = new FormData(document.getElementById("addPatientForm"));
     
        $.ajax({
            cache:false,
            async: false,
            url: 'resources/data/addpatient.php',
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false, 	
                ...
                if (msg.response == "success"){
                    Swal.fire({
                        ...
                        if (result){
                            ...
                            table.ajax.reload();
                            closeModal('addPatientModal', 'addPatientForm')
                        }
                      });
                }
                else{
    ...
    else if (formUpdate == "#editPatientForm"){
        $.ajax({
            cache:false,
            async: false,
            url: 'resources/data/editpatient.php',
            type: 'POST',
            data: dataArray,
            dataType: "JSON",
            success: function(msg) {
                ...
                if (msg.response == "success"){
                    Swal.fire({
                        ...
                        if (result){
                            ...
                            table.ajax.reload();
                            closeModal('editPatientModal', 'editPatientForm')
                        }
                      });
                }
                else{
